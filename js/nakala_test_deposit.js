async function deposerSurNakala(apiKey_in, collection_id_in) {
    ...
    if (window.ptmAuth && window.ptmAuth.isAuthenticated()) {
        isLoggedIn = true;
        try {
            userData = await window.ptmAuth.getUserProfile();
        } catch (error) {
    ...
    const TITLE        = window.metadataDict['Titre'];
    const CREATED_DATE = new Date().toISOString().split('T')[0];
    const LICENSE      = 'CC-BY-4.0';
    const AUTHORS = [window.metadataDict['Cote']];
    ...
    let formattedDate = window.metadataDict['Date'] || '';

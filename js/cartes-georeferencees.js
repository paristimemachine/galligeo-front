/**
 * Gestionnaire de la page des cartes géoréférencées
 * Affiche toutes les cartes géoréférencées par les utilisateurs
 */
class CartesGeoreferencees {
    constructor() {
        this.allMaps = [];
        this.filteredMaps = [];
        this.currentPage = 1;
    ...
    /**
     * Affiche les cartes en mode carte (cards)
     */
    displayAsCards() {
    ...
    /**
     * Affiche les cartes en mode tableau
     */
    displayAsTable() {
        ...
        if (mapsToShow.length === 0) {
            tableBody.innerHTML = `
                <tr>
                    <td colspan="6" class="fr-text--center">
        ...
        const rowsHTML = mapsToShow.map(map => this.generateTableRowHTML(map)).join('');
        tableBody.innerHTML = rowsHTML;
    }

    /**
     * Génère une ligne de tableau pour une carte
     */
    generateTableRowHTML(map) {
        const title = map.title || 'Titre non disponible';
        const creator = map.creator || '';
        const date = map.date || '';
        const arkId = map.ark;
        ...
        return `
            <tr>
                <td>
                    <a href="${georefUrl}" class="fr-link">${title}</a>
                </td>
                <td>${creator}</td>
                <td>${date}</td>
                <td>${georefBy}</td>
            ...
            paginationHTML += `
                <li>
                    <button class="fr-pagination__link fr-pagination__link--prev fr-pagination__link--lg-label" 
                            onclick="window.cartesGeoreferencees.goToPage(${this.currentPage - 1})">
                        Précédent
                    </button>
                </li>
            `;
            ...
            paginationHTML += `
                <li>
                    <button class="fr-pagination__link ${isActive ? 'fr-pagination__link--current' : ''}" 
                            ...
                            onclick="window.cartesGeoreferencees.goToPage(${i})">
                        ${i}
                    </button>
                </li>
            `;
            ...
            <div class="fr-col-12">
                <div class="error-message">
                    <div class="fr-alert fr-alert--error">
                        ...
                        <p>${message}</p>
                        <button class="fr-btn fr-btn--sm fr-mt-2w" onclick="window.cartesGeoreferencees.init()">
                            Réessayer
                        </button>
                    </div>
                </div>
...
// Instance globale
window.cartesGeoreferencees = new CartesGeoreferencees();

// Export pour usage dans d'autres modules si nécessaire
if (typeof module !== 'undefined' && module.exports) {
    module.exports = CartesGeoreferencees;
}